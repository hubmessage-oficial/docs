...
<Note>
  Not every event exists on every channel. Check the [event table by channel](/en/webhooks/introduction#available-events) before configuring a webhook — registering an unavailable event does not return an error, but the webhook will never receive it.
</Note>
