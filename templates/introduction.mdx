...
<Note>
  A gestão de templates desta API é exclusiva de canais **`META_WHATSAPP`**. Instagram, Messenger e Telegram Bot não usam estes endpoints.
</Note>
...
A API organiza templates por **business** (WABA — WhatsApp Business Account). No Hub Message, esse identificador é exposto como `businessId`. Cada business tem seus próprios templates, histórico de aprovação e limites.
