...
description: "Migre da Z-API para o Hub Message sem quebrar suas integrações"
---
...
<Note>
  As únicas mudanças necessárias na sua aplicação são a **base URL** e o **header de autenticação**.
</Note>

| | Antes (Z-API) | Depois (Hub Message) |
|---|---|---|
| **Base URL** | `https://api.z-api.io` | `https://zapi.hubmessage.io` |
| **Autenticação** | Header `Client-Token` | Header `Authorization: Bearer {secret}` |
| **Nomenclatura** | `instance` / `instance_id` | `channel` / `channel_id` |
| **Path da URL** | `/instances/{id}/token/{token}` | `/instances/` ou `/channels/` (ambos funcionam) |
| **Payloads das APIs** | — | Mesma estrutura |
| **Body dos Webhooks** | — | Mesma estrutura |
...
O secret para o header `Authorization` é gerado no painel do Hub Message ao criar sua instância. Os payloads de envio e os bodys dos webhooks mantêm exatamente a mesma estrutura, garantindo que suas integrações continuem funcionando sem quebras.
...
<Steps>
  <Step title="Crie sua conta no Hub Message">
